...
Omega ns t w = (1 n : QNat) -> ((0 prf : IsElem n ns) => Mu n t w)
...
app omega_f omega_x n @{prf} = let 
    ...
    in rewrite sym n0.prf in Mu.app (omega_f n0.val @{prf0}) (rewrite CloneEq {a=n0} in (omega_x n1.val @{prf1}))

public export
combine : Omega p t w -@ Omega q t w -@ Omega (p |+| q) t w
combine omega1 omega2 n @{prf} = let
  1 (For x $ Elem y prf') = App2ElemExists {z=n} @{prf}
  0 prfX = prf'.fst
  0 prfY = prf'.snd.fst
  0 prfZ = prf'.snd.snd
  1 mu1 : Mu x t w = omega1 x @{prfX}
  1 mu2 : Mu y t w = omega2 y @{prfY}
