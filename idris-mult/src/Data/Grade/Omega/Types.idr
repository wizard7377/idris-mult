...
mutual
    ...
    data Omega : (p : QList Form) -> (t : Type) -> (w : t) -> Type where
        ...
        OmegaVal : Mu n (Omega p t w) _ -@ Omega (FVal n :: p) t w
        ...
        Combine : 
          Omega (p :: r) t w -@ 
          Omega (q :: r) t w -@ 
          Omega ((FAdd p q) :: r) t w

        Join : 
          Omega (p :: q :: r) t w -@
          Omega ((FMul p q) :: r) t w 
        Alt : 
            ((1 p : Form) ->
            (Either (p = q) (p = r)) |-
            Omega (p :: s) t w) -@ 
            Omega (FAlt q r :: s) t w

contract_omega_full_1 : y = Omega0 w
%hint export
ContractOmega : {1 p : QList Form} -> (1 w : t) => Contractible (Omega p t w)
ContractOmega {p = []} @{w} = contract @{Omega0 w} @{ contract_omega_full_1 }
ContractOmega {p = (FTop :: xs)} @{w} = contract @{ ?contract_omega_ind_1 } @{ ?contract_omega_ind_prf_1 }
ContractOmega {p = (FBot :: xs)} @{w} = contract @{ ?contract_omega_ind_2 } @{ ?contract_omega_ind_prf_2 }
ContractOmega {p = (FVal n :: xs)} @{w} = contract @{ ?contract_omega_ind_3 } @{ ?contract_omega_ind_prf_3}
ContractOmega {p = (FAlt p q :: xs)} @{w} = contract @{ ?contract_omega_ind_4 } @{ ?contract_omega_ind_prf _4}
ContractOmega {p = (FAdd p q :: xs)} @{w} = contract @{ ?contract_omega_ind_5 } @{ ?contract_omega_ind_prf _5}
ContractOmega {p = (FMul p q :: xs)} @{w} = contract @{ ?contract_omega_ind_6 } @{ ?contract_omega_ind_prf _6}
{-
export 
