...
qfin {hi} k @{prf} = Element k prf

public export
weaken : (0 prf : LLTE hi hi') => QFin hi -@ QFin hi'
weaken @{prf} (Element k prfK) = Element k (lte_trans @{prfK} @{prf})
public export
(.val) : {0 hi : QNat} -> QFin hi -@ QNat
(.val) {hi} (Element k prf) = k
...
ZeroBound (Evidence (Element Zero LLTE_Z) pZ) = pZ
public export
0 AddProof : Dec (prop (Succ hi)) -> Dec (Bounded hi prop) -> Dec (Bounded (Succ hi) prop)
AddProof (Yes pSucc) _ = Yes (Evidence (Element (Succ hi) (lte_refl {k=Succ hi})) pSucc)
AddProof (No nSucc) (Yes (Evidence (Element k prfK) pK)) = Yes (
    Evidence (Element k ?h0) ?h1)
AddProof (No nSucc) (No nBound) = No contra 
  where 
    contra : (Bounded (Succ hi) prop) -> Void
    contra (Evidence (Element k prfK) pK) = case decEq k (Succ hi) of 
...
Limited ask Zero = case ask Zero of 
    Yes pp => Yes (Evidence (Element Zero LLTE_Z) pp)
...
finMinus Zero (Element Zero prf) = Zero
finMinus (Succ x') (Element (Succ k) prf) = finMinus x' (Element k (succ_lte @{prf}))
finMinus (Succ x') (Element Zero prf) = (Succ x')
