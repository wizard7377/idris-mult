...
public export
[CopyClone] Copy a => Duplicable a where 
   duplicate x = copyWithEq go x
     where 
       go : (1 x0 : a) -> (1 x1 : a) -> (0 prf : x0 === x1) => (2 `Copies` x0)
       go x0 x1 @{prf} = (x0 :: (the (1 `Copies` x0) (rewrite prf in x1 :: Nil)))
